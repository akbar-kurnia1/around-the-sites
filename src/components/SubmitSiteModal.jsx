...
export default function SubmitSiteModal({ onClose, onSubmit }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-primary/80 backdrop-blur-md animate-fade-in overflow-y-auto" onClick={onClose}>
      <div className="bg-primary border-4 border-accent p-8 sm:p-10 rounded-[2rem] max-w-4xl w-full shadow-[12px_12px_0_0] shadow-accent relative transform transition-all my-8" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <textarea required name="description" value={formData.description} onChange={handleChange} className="w-full h-full min-h-[120px] bg-primary border-2 border-accent/60 rounded-xl px-4 py-3 text-accent focus:outline-none focus:border-accent focus:shadow-[4px_4px_0_0] focus:shadow-accent transition-all font-medium resize-none placeholder-accent/30" placeholder="Why should people visit this site?"></textarea>
            </div>
          </div>
          
          <button type="submit" className="mt-2 w-full py-4 bg-accent border-4 border-accent text-primary font-black text-lg uppercase tracking-widest rounded-xl hover:bg-primary hover:text-accent hover:shadow-[6px_6px_0_0] hover:shadow-accent transition-all duration-200">
            Submit Now
          </button>
        </form>
      </div>
